...
Com.Sp.sum<-aggregate(obsspp[,c('Field', 'Shrub', 'Subcanopy', 'Tree', 'BA')], by=list(obsspp$Observation_Label, obsspp$AcTaxon, obsspp$Simple), FUN=sum) #sum within plot
colnames(Com.Sp.sum)<-c('Observation_Label', 'Species', 'Simple', 'Field', 'Shrub', 'Subcanopy', 'Tree', 'BA') #restore column names
#freq
...
Com.Sp.mean$plot <- paste0('P.',Com.Sp.mean$Observation_Label)
Com.Sp.mean$plot <- str_replace_all(Com.Sp.mean$plot, ' ', '.')
Com.Sp.mean$plot <- str_replace_all(Com.Sp.mean$plot, '-', '.')
Com.Sp.mean$plot <- str_replace_all(Com.Sp.mean$plot, ',', '.')
Com.Sp.mean$plot <- str_replace_all(Com.Sp.mean$plot, ':', '.')
Com.Sp.mean$plot <- str_replace_all(Com.Sp.mean$plot, ';', '.')
...
Com.Sp.groups.sum <- aggregate(Com.Sp.groups[,c('Field', 'Shrub', 'Subcanopy','Tree', 'Total')],
                               by=list(Com.Sp.groups$cluster, Com.Sp.groups$Species), FUN='sum')
colnames(Com.Sp.groups.sum) <- c('cluster', 'Species', 'Field', 'Shrub', 'Subcanopy','Tree', 'Total')
Com.Sp.groups.count <- aggregate(unique(Com.Sp.groups[c('cluster', 'plot')])$plot, 
                                 by=list(unique(Com.Sp.groups[c('cluster', 'plot')])$cluster), FUN='length')
